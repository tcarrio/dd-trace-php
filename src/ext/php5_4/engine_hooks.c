...
static int _dd_begin_fcall_handler(zend_execute_data *execute_data TSRMLS_DC) {
    ...
    if (!_dd_should_trace_call(execute_data, &current_fbc, &dispatch TSRMLS_CC) ||
        ((dispatch->options & DDTRACE_DISPATCH_POSTHOOK) && current_fbc->type == ZEND_INTERNAL_FUNCTION)) {
...
static void (*_dd_prev_execute_internal)(zend_execute_data *, int return_value_used TSRMLS_DC);
void ddtrace_execute_internal(zend_execute_data *execute_data, int return_value_used TSRMLS_DC) {
    ...
    // See execute_internal on PHP 5.4
    zval **return_value_ptr = &(*(temp_variable *)((char *) EX(Ts) + EX(opline)->result.var)).var.ptr;
    // todo: temporary and references?

    zend_op *opline = execute_data ? EX(opline) : NULL;  // ?
    ddtrace_span_t *span = ddtrace_open_span(execute_data, dispatch TSRMLS_CC);

    _dd_prev_execute_internal(execute_data, return_value_used TSRMLS_CC);

    if (span == DDTRACE_G(open_spans_top)) {
        _dd_execute_end_span(execute_data, span, *return_value_ptr, opline TSRMLS_CC);
    } else {
        if (get_dd_trace_debug()) {
            const char *fname = Z_STRVAL(dispatch->function_name);
            ddtrace_log_errf("Cannot run tracing closure for %s(); spans out of sync", fname);
